// Create a class for the custom element
// This will be an autonomous element
class AlertMessageWebComponent extends HTMLElement {
  constructor() {
    // Always call super first in constructor
    super();
  }

  // Which attributes will trigger attributeChangedCallback
  static get observedAttributes() {
      return ["visible"];
  }

  static createTemplate() {
    let template = document.createElement('div')
    template.setAttribute('id', 'alert-message-template')
    let outerDiv = document.createElement('div')
    outerDiv.setAttribute('class', 'alert')
    let innerDiv = document.createElement('div')
    innerDiv.setAttribute('class', 'error')
    const slot = document.createElement('slot')

    innerDiv.appendChild(slot)
    outerDiv.appendChild(innerDiv)
    template.appendChild(outerDiv)

    return template;
  }
  // ***************************************
  // Custom element lifecycle functions
  // ***************************************
  connectedCallback() {
    let shadowRoot = this.attachShadow({mode: "open"})

    // var style = document.createElement('style');
    // style.textContent = `
    //   alert-message-wc {
    //     display : none;
    //   }
    //
    //   alert-message-wc[visible] {
    //     display : block;
    //     cursor : pointer;
    //   }
    //
    //   .alert {
    //     font-family: Roboto,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif;
    //   }
    //
    //   .alert .error {
    //     padding: .75rem 1.25rem;
    //     color: #990f3d;
    //     background-color: #faf3f5;
    //     border-left: 3em solid;
    //   }
    // `;
    //
    // shadowRoot.appendChild(style);

    let template = AlertMessageWebComponent.createTemplate()
    shadowRoot.appendChild(template);

    let linkElement = document.createElement("link");
    linkElement.setAttribute('rel', 'stylesheet');
    linkElement.setAttribute('href', 'css/alertmessage.css')
    // Apply external class
    shadowRoot.appendChild(linkElement);

    this.addEventListener("click", e => {
      this.visible = false;
    });
  }

  disconnectedCallback() {
    console.log("alert-message disconnected from page");
  }

  adoptedCallback() {
    console.log("alert-message adopted in page");
  }

  attributeChangedCallback(name, oldValue, newValue) {
    console.log("alert-message attr: " + name + " changed from '"
    + oldValue + "' to '" + newValue + "'");
  }
  // Getter for the visible property
  get visible() {
    return this.hasAttribute('visible');
  }

  // Setter for the visible property
  set visible(val) {
    if (val) {
      this.setAttribute('visible', '');
    }
    else {
      this.removeAttribute('visible');
    }
  }
}

// Define the new element
customElements.define('alert-message-wc', AlertMessageWebComponent);
